...
def create_user():
    ...
    if db.session.scalar(sa.select(User).where(User.username == data['username'])):
        return bad_request('Username already taken, please use a different username.')
    if db.session.scalar(sa.select(User).where(User.email == data['email'])):
        return bad_request('Email already in use, please use a different email address.')
    user = User()
...
def update_user(id):
    ...
    if 'email' in data and data['email'] != user.email and \
        ...
        return bad_request('please select a different email address')
