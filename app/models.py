...
class User(UserMixin,db.Model): #db.Model is a base class for all models from FLASK-SQLAlchemy
    ...
    '''so.relationship()'. This is not an actual database field, but a high-level view of the relationship between users and posts
        ...
        The first argument to so.relationship() is the model class that represents the other side of the relationship.
    ...
    #generates hashed password and sets value of password_hash
    def set_password(self, password):
        self.password_hash = generate_password_hash(password)


    #checks if password_hash matches the password entered by the user
    def check_password(self, password):
        return check_password_hash(self.password_hash, password)
    

    #generates avatars for users
    def avatar(self,size):
        digest = md5(self.email.lower().encode('utf-8')).hexdigest()
        #gravtar is used to return the image of an user, the d argument is used to provide imgs for users who dont have one
